...
def load_data():
    try:
        # ⚠️ CORRECTION : On enlève le decimal="," car notre fichier propre utilise des points (.)
        df = pd.read_csv("data_filtered/2022/CLEAN_2_Densite_population_2022.csv", sep=";")
        # Sécurité classique pour le tri
...
if df_global is None:
    st.error("❌ Fichier introuvable. Lance le script 02 de nettoyage d'abord.")
    st.stop()
...
with st.sidebar:
    ...
    region_choisie = st.selectbox("Région :", ["Toute la région étudiée"] + list(liste_regions))
    
    df_filtre = df_global[df_global['region'] == region_choisie] if region_choisie != "Toute la région étudiée" else df_global.copy()
...
# --- EN-TÊTE ---
st.title("🗺️ Analyse de la Densité de Population (2022)")

if df_filtre.empty:
    st.warning("⚠️ Aucune donnée ne correspond aux filtres.")
else:
    # ⚠️ CORRECTION SÉCURITÉ : On s'assure que les colonnes sont bien numériques avant de faire des sommes
    df_filtre['nb_pers'] = pd.to_numeric(df_filtre['nb_pers'], errors='coerce').fillna(0)
    df_filtre['superficie_km2'] = pd.to_numeric(df_filtre['superficie_km2'], errors='coerce').fillna(0)
    df_filtre['densite'] = pd.to_numeric(df_filtre['densite'], errors='coerce').fillna(0)
    ...
    col2.metric("Superficie (km²)", f"{int(total_superficie):,}".replace(",", " "))
    # On affiche une jolie virgule pour la France
    col3.metric("Densité Moyenne", f"{densite_moyenne}".replace(".", ",") + " hab/km²")
    col4.metric("Nb de Communes", f"{len(df_filtre):,}".replace(",", " "))

    st.write("")
    tab1, tab2, tab3 = st.tabs(["📊 Palmarès (Top 10)", "🎯 Grille de Densité", "🗄️ Explorer la Data"])

    with tab1:
        col_gauche, col_droite = st.columns(2)
        with col_gauche:
            st.markdown("**Top 10 des communes par Population**")
            df_pop = df_filtre.nlargest(10, 'nb_pers').sort_values('nb_pers')
            fig_pop = px.bar(df_pop, x='nb_pers', y='localisation', orientation='h', text='nb_pers', color='nb_pers', color_continuous_scale='Blues')
        ...
        with col_droite:
            ...
            df_dense = df_filtre.nlargest(10, 'densite').sort_values('densite')
            fig_dense = px.bar(df_dense, x='densite', y='localisation', orientation='h', color='densite', color_continuous_scale='Reds', text='densite')
    ...
    with tab2:
        ...
        df_grille['Code Grille'] = "Grille Type " + df_grille['Code Grille'].astype(str) # Rend l'affichage plus joli
    ...
    with tab3:
        ...
        # L'export, lui, sort bien avec de vraies virgules pour Excel grâce à decimal=","
        csv = df_filtre.to_csv(index=False, sep=";", decimal=",").encode('utf-8-sig')
        nom_export = f'export_densite_{region_choisie.replace(" ", "_")}.csv' if region_choisie != "Toute la région étudiée" else 'export_densite_region.csv'
